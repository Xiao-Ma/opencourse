...
def spawn_ball(direction):
    global ball_pos, ball_vel, pause # these are vectors stored as lists
    ball_pos = [WIDTH / 2, HEIGHT / 2]
    ball_vel[1] = -random.randrange(60, 240) / 60
    if direction:
...
def draw(canvas):
    ...
    if not pause:
        if (paddle1_vel < 0 and paddle1_pos > HALF_PAD_HEIGHT or
            paddle1_vel > 0 and HEIGHT - paddle1_pos > HALF_PAD_HEIGHT):
            paddle1_pos += paddle1_vel
        if (paddle2_vel < 0 and paddle2_pos > HALF_PAD_HEIGHT or 
            paddle2_vel > 0 and HEIGHT - paddle2_pos > HALF_PAD_HEIGHT):
    ...
    canvas.draw_text(str(score2), [WIDTH - WIDTH / 4, 20], 20, "white")
...
frame.add_label('Press "Space" to start or pause game.')
